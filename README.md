...
조문 상세 페이지에는 **고용노동부 행정해석(질의회시)** 이 함께 붙는다. 회시번호·일자·질의·회답과
국가법령정보센터 원문 링크를 싣는데, **회시번호가 실재하고 내용까지 일치하는 것만** 넣는다.
확인되지 않은 것은 싣지 않고, 대신 국가법령정보센터 '중앙부처 1차 해석' 검색으로 연결한다.
데이터는 `data/platform/interpretations.json`.
