...
조문 상세 페이지에는 **행정해석(질의회시)** 이 함께 붙는다. 140개 조문에 276건이며 대부분 고용노동부,
일부는 법제처·교육부·행정안전부다. 회시번호·일자·질의·회답과 원문 링크를 싣는데, **회시번호가 실재하고
내용까지 일치하는 것만** 넣는다. 확인되지 않은 조문은 싣지 않고 국가법령정보센터 '중앙부처 1차 해석'
검색으로 연결한다. 데이터는 `data/platform/interpretations.json`.

수집 뒤 별도 에이전트가 회시번호 실재·내용 일치·**날짜 정합성**(회시 일자보다 나중에 나온 판례 내용이
붙어 있지 않은지)을 독립 검증한다. 이 단계 없이 수집 결과를 그대로 쓰면 안 된다 — 번호는 진짜인데
내용을 다른 회시에서 가져다 붙이는 오류가 실제로 나온다.

데이터를 새로 만들었다면 국가법령정보센터 링크가 실제로 그 회시번호를 담고 있는지 기계적으로 대조해 볼 것.
...
정적 내보내기(`output: 'export'`)는 **빌드할 때만** 켜진다. dev에서 켜 두면 `/law/근로기준법/제60조/`
처럼 세그먼트가 둘인 한글 경로가 파라미터 검증에 걸려 500이 난다(배포본은 멀쩡하다).
